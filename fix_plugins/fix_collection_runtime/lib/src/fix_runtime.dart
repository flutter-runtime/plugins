...
class FixRuntime {
  ...
  _fixClass(AnalyzerClassCache cache) {
    if (filePath == 'lib/src/iterable_extensions.dart' &&
        cache.name == 'Iterable') {
      cache.methods.removeWhere((element) => element.name == 'sortedBy');
      cache.methods.removeWhere((element) => element.name == 'isSortedBy');
    }
